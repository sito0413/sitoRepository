...
public class XMLBook {
	 ...
	 * コンストラクタ<br>
	 */
	public XMLBook(final URI uri) {
		this(document(uri));
	}

	private static Document document(final URI uri) {
		Document document = null;
		try {
			document = DocumentBuilderFactory.newInstance()
					.newDocumentBuilder().parse(new File(uri));
		}
		catch (Exception exp) {
			// NOP
		}
	...
	public XMLBook(final Document document) {
		worksheetsMap = new ConcurrentHashMap<>();
		stylesMap = new ConcurrentHashMap<>();
		// スタイル.
		parseStyles((Element) document
				.getElementsByTagName(XMLConstants.STYLES).item(0));
		// ワークシート.
	...
	public Map<String, XMLStyle> parseStyles(final Element stylesElement) {
		...
		// スタイルリストを取得.
		NodeList stylesNodeList = stylesElement
				.getElementsByTagName(XMLConstants.STYLE);
		// スタイルごとにオブジェクトを生成.
		for (int i = 0; i < stylesNodeList.getLength(); i++) {
			...
			String parentId = styleElement
					.getAttribute(XMLConstants.ATTR_PARENT_ID);
			if (parentId.isEmpty()) {
				// ---[ Note ]---------------------------------------
			...
			// スタイル生成＆属性設定.
			XMLStyle newStyle = new XMLStyle(stylesMap.get(parentId),
					styleElement);
			// スタイル追加
	 ...
	 * @return
	 */
	public Map<String, XMLSheet> parseWorksheets(
			final NodeList worksheetsNodeList) {
		...
		for (int i = 0; i < worksheetsNodeList.getLength(); i++) {
			...
			String worksheetName = worksheetElement
					.getAttribute(XMLConstants.ATTR_NAME);
			if (worksheetElement.getElementsByTagName(XMLConstants.ELM_TABLE)
					.item(0) == null) {
				continue;
			}
			// ワークシート生成＆属性設定.

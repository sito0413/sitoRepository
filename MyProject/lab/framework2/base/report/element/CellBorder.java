...
public class CellBorder implements Border {
	List<BorderInfo> borderInfos;

	public CellBorder(final List<BorderInfo> borderInfos) {
		...
		topDrawPoints.add(new Point(0, 0));
		topDrawPoints.add(new Point(0, 0));
		leftDrawPoints.add(new Point(0, 0));
		leftDrawPoints.add(new Point(0, 0));
		rightDrawPoints.add(new Point(0, 0));
		rightDrawPoints.add(new Point(0, 0));
		bottomDrawPoints.add(new Point(0, 0));
		bottomDrawPoints.add(new Point(0, 0));
		dLeftDrawPoints.add(new Point(0, 0));
		dLeftDrawPoints.add(new Point(0, 0));
		dRightDrawPoints.add(new Point(0, 0));
		dRightDrawPoints.add(new Point(0, 0));
	}

	public void paintBorder(final Component component, final Graphics g, final int i, final int j, final int k, final int l) {
		...
		createDrawPoints(k - 1, l - 1);
		for (int index = 0; index < borderInfos.size(); index++) {
			drowLine(g2D, k, l, borderInfos.get(index));
		}
	}
	...
	private void drowLine(final Graphics2D g2D, final int width, final int height, final BorderInfo entry) {
		...
		Point start = drawPoints.get(START_POINT_INDEX);
		Point end = drawPoints.get(END_POINT_INDEX);
	...
	private void createDrawPoints(final int newX, final int newY) {
		topDrawPoints.get(1).setLocation(newX, 0);
		leftDrawPoints.get(1).setLocation(0, newY);
		rightDrawPoints.get(0).setLocation(newX, 0);
		rightDrawPoints.get(1).setLocation(newX, newY);
		bottomDrawPoints.get(0).setLocation(0, newY);
		bottomDrawPoints.get(1).setLocation(newX, newY);
		dLeftDrawPoints.get(1).setLocation(newX, newY);
		dRightDrawPoints.get(0).setLocation(0, newY);
		dRightDrawPoints.get(1).setLocation(newX, 0);
	}

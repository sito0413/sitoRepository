...
public class XMLRow {
	 ...
	 * @return
	 */
	public XMLRow(final Node rowNode, final int index,
		...
		// インデックス.
		this.index = (parseInt(
				rowElement.getAttribute(XMLConstants.ATTR_INDEX), index));
		// スパン.
		this.span = (parseInt(rowElement.getAttribute(XMLConstants.ATTR_SPAN),
				0));
		// 行高.
		this.height = (parseDouble(
				rowElement.getAttribute(XMLConstants.ATTR_HEIGHT),
				defaultHeight));
		// スタイルID
		this.styleId = (rowElement.getAttribute(XMLConstants.ATTR_STYLEID));
		// セル.
		this.cells = new CopyOnWriteArrayList<>();
		this.parseAndSetCells(rowElement
				.getElementsByTagName(XMLConstants.ELM_CELL));
	}
	...
	public XMLCell getCell(final int columnIndex) {
		while (columnIndex >= getCells().size()) {
			getCells().add(null);
		}
	...
	private void parseAndSetCells(final NodeList cellsNodeList) {
		...
		for (int i = 0; i < cellsNodeList.getLength(); i++) {
			...
			// 値.
			NodeList dataNodeList = cellElement
					.getElementsByTagName(XMLConstants.ELM_DATA);

...
public class XMLStyle {
	...
	public XMLStyle(final XMLStyle referStyle, final Element styleElement) {
		...
		this.parseAndSetAlignments((Element) styleElement.getElementsByTagName(XMLConstants.STYLE_ALIGNMENT).item(0));
		this.parseAndSetBorders(styleElement.getElementsByTagName(XMLConstants.STYLE_BORDER));
		this.parseAndSetFont((Element) styleElement.getElementsByTagName(XMLConstants.STYLE_FONT).item(0));
	}
	...
	private void initialize(final XMLStyle referStyle) {
		if (referStyle == null) {
			...
			borders = new CopyOnWriteArrayList<XMLBorder>();
			...
			borders = new CopyOnWriteArrayList<XMLBorder>(referStyle.getBorders());
			font = referStyle.getFont().deriveFont(referStyle.getFont().getStyle());
		}
	}
	...
	private void parseAndSetAlignments(final Element alignmentsElement) {
		...
		// 横位置.
		horizontalAlignment = XMLHorizontalAlignment.getDefaultValue();
		String horizontalAttribute = alignmentsElement.getAttribute(XMLConstants.ATTR_HORIZONAL);
		if ((horizontalAttribute != null) && (!horizontalAttribute.isEmpty())) {
			horizontalAlignment = XMLHorizontalAlignment.parse(alignmentsElement.getAttribute(XMLConstants.ATTR_HORIZONAL));
		}

		// 縦位置.
		verticalAlignment = XMLVerticalAlignment.getDefaultValue();
		String verticalAttribute = alignmentsElement.getAttribute(XMLConstants.ATTR_VERTICAL);
		if ((verticalAttribute != null) && (!verticalAttribute.isEmpty())) {
			verticalAlignment = XMLVerticalAlignment.parse(alignmentsElement.getAttribute(XMLConstants.ATTR_VERTICAL));
		}

		// 縮小表示.
		shrinkToFit = false;
		String shrinkToFitAttribute = alignmentsElement.getAttribute(XMLConstants.ATTR_SHRINKTOFIT);
		if ((shrinkToFitAttribute != null) && (!shrinkToFitAttribute.isEmpty())) {
			Object object = alignmentsElement.getAttribute(XMLConstants.ATTR_SHRINKTOFIT);
			if (object != null) {
	...
	private void parseAndSetFont(final Element fontElement) {
		...
		// フォント名.
		String fontName = ARIAL.getName();
		String fontNamettribute = fontElement.getAttribute(XMLConstants.ATTR_FONTNAME);
		if ((fontNamettribute != null) && (!fontNamettribute.isEmpty())) {
			fontName = fontNamettribute;
		}

		// ボールド.
		boolean bold = ARIAL.isBold();
		...
		// イタリック.
		boolean italic = ARIAL.isItalic();
		String italicAttribute = fontElement.getAttribute(XMLConstants.ATTR_ITALIC);
		...
		// フォントサイズ.
		float fontSize = -1f;
		String fontSizeAttribute = fontElement.getAttribute(XMLConstants.ATTR_SIZE);
		...
		if (fontSize < 0f) {
			fontSize = ARIAL.getSize();
		}
		// 新しいフォントの生成.
		Font newFont = Font.decode(fontName).deriveFont((bold ? Font.BOLD : Font.PLAIN) + (italic ? Font.ITALIC : Font.PLAIN),
		        fontSize);
		// フォントの更新.

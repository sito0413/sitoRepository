...
public class FROM implements SQL<SelectSQL> {
	...
	public FROM(final String sql, final Table<?> table) {
		this(sql, table.name);
	}
	
	public FROM(final String sql, final SelectSQL selectSQL) {
		this(sql, "(" + selectSQL + ")");
	}
	
	public FROM(final String sql, final String str) {
		base = sql + "FROM " + str + " ";

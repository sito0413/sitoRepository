...
public class XMLTable {
	...
	public XMLTable(final Node tableNode) {
		...
		// 標準列幅.
		defaultColumnWidth = (parseDouble(
				tableElement.getAttribute(XMLConstants.ATTR_DEFCOLUMNWIDTH),
				XMLTable.DEFAULT_COLUMN_WIDTH));
		// 標準行高さ.
		defaultRowHeight = (parseDouble(
				tableElement.getAttribute(XMLConstants.ATTR_DEFROWHEIGHT),
				XMLTable.DEFAULT_ROW_HEIGHT));
		// 列数.
		columnCount = (parseInt(
				tableElement.getAttribute(XMLConstants.ATTR_EXCOLUMNCOUNT), 0));
		// 行数.
		rowCount = (parseInt(
				tableElement.getAttribute(XMLConstants.ATTR_EXROWCOUNT), 0));
		// 列.
		this.parseAndSetColumns(tableElement
				.getElementsByTagName(XMLConstants.ELM_COLUMN));
		// 行.
		this.parseAndSetRows(tableElement
				.getElementsByTagName(XMLConstants.ELM_ROW));
	}
	...
	public Set<XMLColumn> getColumns() {
		if (this.columns == null) {
			this.columns = new CopyOnWriteArraySet<>();
		}
		return this.columns;
	}
	...
	public Set<XMLRow> getRows() {
		if (this.rows == null) {
			this.rows = new CopyOnWriteArraySet<>();
		}
		return this.rows;
	}
	...
	private void parseAndSetColumns(final NodeList columnsNodeList) {
		...
		for (int i = 0; i < columnsNodeList.getLength(); i++) {
			// 列情報変換.
			XMLColumn newColumn = new XMLColumn(columnsNodeList.item(i),
					index + 1, getDefaultColumnWidth());
			// 列情報補完.
			// - 列情報が明示的に存在しない場合、列情報を補完する.
			if (index < newColumn.getIndex() - 1) {
				int completeIndex = index + 1;
				getColumns().add(
						new XMLColumn(completeIndex, newColumn.getIndex()
								- completeIndex - 1, getDefaultColumnWidth()));
			}
			// インデックス更新.
		...
		if (index < columnCount) {
			getColumns().add(
					new XMLColumn(index + 1, columnCount - index - 1,
	...
	private void parseAndSetRows(final NodeList rowsNodeList) {
		...
		for (int i = 0; i < rowsNodeList.getLength(); i++) {
			// 行情報変換.
			XMLRow newRow = new XMLRow(rowsNodeList.item(i), index + 1,
					getDefaultRowHeight());
			// インデックス更新.
			...
			if ((i == 0) && (newRow.getIndex() > 1)) {
				getRows().add(
						new XMLRow(1, newRow.getIndex() - 2,
								getDefaultRowHeight()));
			}
			// 行情報追加.
			getRows().add(newRow);
		...
		if (index < rowCount) {
			getRows().add(
					new XMLRow(index + 1, rowCount - index - 1,
							getDefaultRowHeight()));
		}
	}
